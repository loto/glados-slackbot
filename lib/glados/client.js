...
async function conversationStart(uuid, remainingAttempts = 3) {
    ...
    try {
        ...
        if (remainingAttempts === 0) throw new errors.TooManyAttemptsError(error, 'Glados: conversation start');
...
async function conversationSay(token, message, remainingAttempts = 3) {
    ...
    try {
        ...
        if (remainingAttempts === 0) throw new errors.TooManyAttemptsError(error, 'Glados: conversation say');
...
async function agentsList(remainingAttempts = 3) {
    ...
    try {
        ...
        if (remainingAttempts === 0) throw new errors.TooManyAttemptsError(error, 'Glados: agents list');
...
async function agentSelect(token, name, remainingAttempts = 3) {
    ...
    try {
        ...
        if (remainingAttempts === 0) throw new errors.TooManyAttemptsError(error, 'Glados: agent select');
